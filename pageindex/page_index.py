...
################### check title in page #########################################################
async def check_title_appearance(item, page_list, start_index=1, model=None, opt=None):    
    title=item['title']
    ...
    response = await ChatGPT_API_async(model=model, prompt=prompt, opt=opt)
...
async def check_title_appearance_in_start(title, page_text, model=None, logger=None, opt=None):    
    prompt = f"""
...
async def check_title_appearance_in_start_concurrent(structure, page_list, model=None, logger=None, opt=None):
    if logger:
    ...
    for item in structure:
        if item.get('physical_index') is not None:
            ...
            tasks.append(check_title_appearance_in_start(item['title'], page_text, model=model, logger=logger, opt=opt))
...
def toc_detector_single_page(content, model=None, opt=None):
    prompt = f"""
    ...
    response = ChatGPT_API(model=model, prompt=prompt, opt=opt)
    # print('response', response)
    json_content = extract_json(response)    
    return json_content['toc_detected']


def check_if_toc_extraction_is_complete(content, toc, model=None, opt=None):
    prompt = f"""
...
def check_if_toc_transformation_is_complete(content, toc, model=None, opt=None):
    prompt = f"""
...
def extract_toc_content(content, model=None, opt=None):
    prompt = f"""
    ...
    response, finish_reason = ChatGPT_API_with_finish_reason(model=model, prompt=prompt, opt=opt)
    
    if_complete = check_if_toc_transformation_is_complete(content, response, model, opt=opt)
    ...
    new_response, finish_reason = ChatGPT_API_with_finish_reason(model=model, prompt=prompt, chat_history=chat_history, opt=opt)
...
def detect_page_index(toc_content, model=None, opt=None):
...
def toc_extractor(page_list, toc_page_list, model, opt=None):
    ...
    has_page_index = detect_page_index(toc_content, model=model, opt=opt)
...
def toc_transformer(toc_content, model=None, opt=None):
    ...
    last_complete, finish_reason = ChatGPT_API_with_finish_reason(model=model, prompt=prompt, opt=opt)
    if_complete = check_if_toc_transformation_is_complete(toc_content, last_complete, model, opt=opt)
    if if_complete == "yes" and finish_reason == "finished":
    ...
    while not (if_complete == "yes" and finish_reason == "finished"):
        ...
        new_complete, finish_reason = ChatGPT_API_with_finish_reason(model=model, prompt=prompt, opt=opt)
...
def find_toc_pages(start_page_index, page_list, opt, logger=None):
    ...
    while i < len(page_list):
        ...
        if i >= opt.toc_check_page_num and not last_page_is_yes:
            break
        detected_result = toc_detector_single_page(page_list[i][0], model=opt.model, opt=opt)
...
def add_page_number_to_toc(part, structure, model=None, opt=None):
    fill_prompt_seq = """
    ...
    current_json_raw = ChatGPT_API(model=model, prompt=prompt, opt=opt)
...
def generate_toc_continue(toc_content, part, model="gpt-4o-2024-11-20", opt=None):
...
### add verify completeness
def generate_toc_init(part, model=None, opt=None):
...
def process_no_toc(page_list, start_index=1, model=None, logger=None, opt=None):
    ...
    toc_with_page_number= generate_toc_init(group_texts[0], model, opt=opt)
    for group_text in group_texts[1:]:
        toc_with_page_number_additional = generate_toc_continue(toc_with_page_number, group_text, model, opt=opt)    
...
def process_toc_no_page_numbers(toc_content, toc_page_list, page_list,  start_index=1, model=None, logger=None, opt=None):
    page_contents=[]
    token_lengths=[]
    toc_content = toc_transformer(toc_content, model, opt=opt)
    ...
    for group_text in group_texts:
        toc_with_page_number = add_page_number_to_toc(group_text, toc_with_page_number, model, opt=opt)
...
def process_toc_with_page_numbers(toc_content, toc_page_list, page_list, toc_check_page_num=None, model=None, logger=None, opt=None):
    toc_with_page_number = toc_transformer(toc_content, model, opt=opt)
    ...
    toc_with_physical_index = toc_index_extractor(toc_no_page_number, main_content, model, opt=opt)
    ...
    toc_with_page_number = process_none_page_numbers(toc_with_page_number, page_list, model=model, opt=opt)
    logger.info(f'toc_with_page_number: {toc_with_page_number}')
...
def process_none_page_numbers(toc_items, page_list, start_index=1, model=None, opt=None):
    for i, item in enumerate(toc_items):
        if "physical_index" not in item:
            ...
            del item_copy['page']
            result = add_page_number_to_toc(page_contents, item_copy, model, opt=opt)
...
def check_toc(page_list, opt=None):
    ...
    if len(toc_page_list) == 0:
        ...
        print('toc found')
        toc_json = toc_extractor(page_list, toc_page_list, opt.model, opt=opt)
        ...
        if toc_json['page_index_given_in_toc'] == 'yes':
            ...
            while (toc_json['page_index_given_in_toc'] == 'no' and 
                ...
                additional_toc_json = toc_extractor(page_list, additional_toc_pages, opt.model, opt=opt)
...
################### fix incorrect toc #########################################################
def single_toc_item_index_fixer(section_title, content, model="gpt-4o-2024-11-20", opt=None):
    tob_extractor_prompt = """
...
async def fix_incorrect_toc(toc_with_page_number, page_list, incorrect_results, start_index=1, model=None, logger=None, opt=None):
    ...
    async def process_and_check_item(incorrect_item):
        ...
        physical_index_int = single_toc_item_index_fixer(incorrect_item['title'], content_range, model, opt=opt)
        
        # Check if the result is correct
        check_item = incorrect_item.copy()
        check_item['physical_index'] = physical_index_int
        check_result = await check_title_appearance(check_item, page_list, start_index, model, opt=opt)
...
async def fix_incorrect_toc_with_retries(toc_with_page_number, page_list, incorrect_results, start_index=1, max_attempts=3, model=None, logger=None, opt=None):
    ...
    while current_incorrect:
        ...
        current_toc, current_incorrect = await fix_incorrect_toc(current_toc, page_list, current_incorrect, start_index, model, logger, opt=opt)
...
################### verify toc #########################################################
async def verify_toc(page_list, list_result, start_index=1, N=None, model=None, opt=None):
    ...
    tasks = [
        check_title_appearance(item, page_list, start_index, model, opt=opt)
...
async def meta_processor(page_list, mode=None, toc_content=None, toc_page_list=None, start_index=1, opt=None, logger=None):
    ...
    if mode == 'process_toc_with_page_numbers':
        toc_with_page_number = process_toc_with_page_numbers(toc_content, toc_page_list, page_list, toc_check_page_num=opt.toc_check_page_num, model=opt.model, logger=logger, opt=opt)
    ...
    accuracy, incorrect_results = await verify_toc(page_list, toc_with_page_number, start_index=start_index, model=opt.model, opt=opt)
    ...
    if accuracy > 0.6 and len(incorrect_results) > 0:
        toc_with_page_number, incorrect_results = await fix_incorrect_toc_with_retries(toc_with_page_number, page_list, incorrect_results,start_index=start_index, max_attempts=3, model=opt.model, logger=logger, opt=opt)
        return toc_with_page_number
    else:
        if mode == 'process_toc_with_page_numbers':
...
async def process_large_node_recursively(node, page_list, opt=None, logger=None):
    ...
    if node['end_index'] - node['start_index'] > opt.max_page_num_each_node and token_num >= opt.max_token_num_each_node:
        ...
        node_toc_tree = await check_title_appearance_in_start_concurrent(node_toc_tree, page_list, model=opt.model, logger=logger, opt=opt)
...
async def tree_parser(page_list, opt, doc=None, logger=None):
    ...
    toc_with_page_number = await check_title_appearance_in_start_concurrent(toc_with_page_number, page_list, model=opt.model, logger=logger, opt=opt)
    ...
    async def page_index_builder():
        ...
        if opt.if_add_node_summary == 'yes':
            if opt.if_add_node_text == 'no':
                add_node_text(structure, page_list)
            await generate_summaries_for_structure(structure, model=opt.model, opt=opt)
            if opt.if_add_node_text == 'no':
                remove_structure_text(structure)
            if opt.if_add_doc_description == 'yes':
                # Create a clean structure without unnecessary fields for description generation
                clean_structure = create_clean_structure_for_description(structure)
                doc_description = generate_doc_description(clean_structure, model=opt.model, opt=opt)
                return {
