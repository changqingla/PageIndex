...
async def get_node_summary(node, summary_token_threshold=200, model=None, opt=None):
...
async def generate_summaries_for_structure_md(structure, summary_token_threshold, model=None, opt=None):
    nodes = structure_to_list(structure)
    tasks = [get_node_summary(node, summary_token_threshold=summary_token_threshold, model=model, opt=opt) for node in nodes]
...
async def md_to_tree(md_path, if_thinning=False, min_token_threshold=None, if_add_node_summary='no', summary_token_threshold=None, model=None, if_add_doc_description='no', if_add_node_text='no', if_add_node_id='yes', opt=None):
    ...
    if if_add_node_summary == 'yes':
        ...
        tree_structure = await generate_summaries_for_structure_md(tree_structure, summary_token_threshold=summary_token_threshold, model=model, opt=opt)
        ...
        if if_add_doc_description == 'yes':
            ...
            doc_description = generate_doc_description(clean_structure, model=model, opt=opt)
            return {
